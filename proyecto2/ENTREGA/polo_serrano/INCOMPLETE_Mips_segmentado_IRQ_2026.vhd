...
begin
	...
	Banco_EX_MEM: Banco_MEM PORT MAP ( 	ALU_out_EX => ALU_out_EX, ALU_out_MEM => ALU_out_MEM, clk => clk, reset => reset_MEM, load => load_MEM, MemWrite_EX => MemWrite_EX,
								...
								-- Use the forwarded B operand so sw can receive the correct rt value
								-- from the forwarding network before it is written to memory.
								BusB_EX => Mux_B_out,  
										--FIN COMPLETAR
...
-------------------------------------------------------------------------------------------------- 
	...
	inc_cycles <= '1';--Done
	inc_I <= valid_I_MEM and not(stall_MIPS); --Complete. Count each valid instruction once when it advances from MEM to WB. 
	inc_data_stalls <= stall_ID and not(stall_MIPS); --Complete. Stall cycles due to data hazards
	inc_control_stalls <= kill_IF; --Complete.Stall cycles due to control hazards
	inc_Exceptions <= Exception_accepted;--Complete. Total number of accepted exceptions
	inc_Mem_stalls <= stall_MIPS and IO_MEM_ready and not(ALU_ready); --Complete. Structural Stall cycles due to multi-cycle memory references
	inc_EX_stalls <= stall_MIPS and not(IO_MEM_ready) and valid_I_MEM; --Complete. Structural Stall cycles generated by the multi-cycle MAC operations
