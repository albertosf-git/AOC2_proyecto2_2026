...
-- Data hazards:
	-- 1) lw_uso:
	--    * rs is needed by the ALU stage.
	--    * rt is also needed by SW, but the store data is captured in ID and there is no forwarding path to MEM.
	ld_uso_rs <= 	'1' when (MemRead_EX = '1' and valid_I_EX = '1' and valid_I_ID = '1' and (Reg_Rs_ID = RW_EX) and (rs_read = '1')) else '0';
	ld_uso_rt <= 	'1' when (MemRead_EX = '1' and valid_I_EX = '1' and valid_I_ID = '1' and (Reg_Rt_ID = RW_EX) and (rt_read = '1')) else '0';
...
-- stall_MIPS_internal is defined to be readable in the code (in vhdl the outputs of an entity cannot be read inside the entity. 
	stall_MIPS_internal <= (not(IO_MEM_ready) and valid_I_MEM) or not(ALU_ready);
